...
This guide covers how to set up the development environment and build SuperFlow from source across different platforms.
...
```bash
git clone git@github.com:cjpais/SuperFlow.git
cd SuperFlow
```
...
The raw binary (`src-tauri/target/release/superflow`) cannot run standalone — it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.

**Install from the deb bundle** (works on any Linux distro):

```bash
cd /tmp
ar x /path/to/SuperFlow/src-tauri/target/release/bundle/deb/SuperFlow_*_amd64.deb data.tar.gz
tar xzf data.tar.gz
sudo cp usr/bin/superflow /usr/bin/
sudo cp -a usr/lib/. /usr/lib/
sudo cp -r usr/share/icons/hicolor/* /usr/share/icons/hicolor/
sudo cp usr/share/applications/SuperFlow.desktop /usr/share/applications/
```

The runtime libraries live in the app-private `/usr/lib/SuperFlow/` (on the binary's rpath), so no `ldconfig` step is needed.

After subsequent rebuilds, copy the binary and any refreshed runtime libraries:

```bash
sudo cp src-tauri/target/release/superflow /usr/bin/
sudo mkdir -p /usr/lib/SuperFlow
sudo cp -a src-tauri/transcribe-libs/. /usr/lib/SuperFlow/
```
...
remains visibly enabled, leaving SuperFlow on `Waiting...`.

After installing the final bundle at `/Applications/SuperFlow.app`, quit SuperFlow, clear only its
stale Accessibility record, then reopen it:

```bash
osascript -e 'tell application id "com.pais.superflow" to quit' || true
tccutil reset Accessibility com.pais.superflow
open /Applications/SuperFlow.app
```
...
```bash
codesign -dr - /path/to/previous/SuperFlow.app 2>&1
codesign -dr - /Applications/SuperFlow.app 2>&1
```
...
See [issue #1618](https://github.com/cjpais/SuperFlow/issues/1618) for the related onboarding
...
```
Bundling SuperFlow_*_amd64.AppImage
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir SuperFlow.AppDir --plugin gtk --output appimage
```
...
# Rust projects' build output, not just SuperFlow):
...
If the build compiles all the way to `Built application at: ...\superflow.exe` and
then fails with:

```
Signing C:\...\superflow.exe with a custom signing command
