...
- **Picker:** custom `PocketVoicePicker` (native React, no base-ui). Trigger is `Button variant="secondary" size="sm"` full width (`!h-9 !justify-between !px-3`), leading orb `size-6`, name + trailing `ChevronsUpDown size-4 opacity-50`. Panel absolute `rounded-lg border`, dark `border-white/[0.06] bg-[#363230]` / light `border-stone-200/70 bg-white`, search input + `max-h-60` list. Rows: real `Orb size-8` (stable `seed` per voice) + name `text-sm font-medium truncate` + `Check size-4` when selected (`opacity-0` otherwise). Row hover/selected: dark `bg-white/[0.08]` / light `bg-stone-100/80`.
- **Voices:** pulled from backend `tts_voices` (`alba, marius, javert, jean, fantine, cosette, eponine, azelma`), selected persisted via `tts_selected_voice` / `tts_set_voice`. Synthesis reads stored voice; default `alba` keeps the legacy no-flag path.
