...
const components: Components = {
  ...
  br: () => <br />,
  hr: () => <hr className="border-stone-700" />,
  img: ({ alt, src }) => {
  ...
  code: ({ children, className }) => {
    ...
    return (
      <code className="rounded bg-stone-900 px-1 py-0.5 font-mono text-[0.85em]">
        {children}
      </code>
    );
  },
  pre: ({ children }) => (
    <pre className="overflow-x-auto rounded-md bg-stone-900 p-3 text-xs leading-relaxed text-text/80">
      {children}
    </pre>
  ),
  a: ({ children, href }) => {
    ...
    return (
      <a
        ...
        className="text-blue-500 underline decoration-logo-primary/40 underline-offset-2 hover:decoration-logo-primary"
      >
        {children}
      </a>
    );
