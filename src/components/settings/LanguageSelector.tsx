    ...
    >
      <div className="flex items-center space-x-1">
        <div className="relative" ref={dropdownRef}>
          <button
            type="button"
            className={`px-2 py-1 text-sm font-semibold bg-stone-900 border border-stone-700 rounded min-w-[200px] text-start flex items-center justify-between transition-all duration-150 ${
              isUpdating("selected_language")
                ? "opacity-50 cursor-not-allowed"
                : "hover:bg-surface-hover cursor-pointer hover:border-stone-600"
            }`}
          ...
          {isOpen && !isUpdating("selected_language") && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-stone-200 rounded shadow-none z-50 max-h-60 overflow-hidden dark:bg-surface dark:border-stone-700 dark:shadow-none">
              {/* Search input */}
              <div className="p-2 border-b border-stone-700">
                <input
                  ...
                  className="w-full px-2 py-1 text-sm bg-stone-900 border border-stone-700 rounded focus:outline-none focus:ring-1 focus:ring-blue-600 focus:border-blue-600"
                />
              </div>
                ...
                {filteredLanguages.length === 0 ? (
                  ...
                  filteredLanguages.map((language) => (
                    <button
                      key={language.value}
                      type="button"
                      className={`w-full px-2 py-1 text-sm text-start hover:bg-surface-hover transition-colors duration-150 ${
      ...
      {isUpdating("selected_language") && (
        <div className="absolute inset-0 bg-stone-900 rounded flex items-center justify-center">
