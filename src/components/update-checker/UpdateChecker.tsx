...
const UpdateChecker: React.FC<UpdateCheckerProps> = ({ className = "" }) => {
  ...
  useEffect(() => {
    ...
    // Dev builds have no release infrastructure — never hit the updater there.
    ...
    <>
      {showPortableUpdateDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-surface border border-stone-700 rounded-lg p-6 max-w-md w-full mx-4 space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              <button
                className="px-3 py-1.5 text-sm rounded border border-stone-700 hover:bg-stone-900 transition-colors"
                onClick={() => setShowPortableUpdateDialog(false)}
              >
                {t("common.close")}
              </button>
              <button
                className="px-3 py-1.5 text-sm rounded bg-blue-600 text-white hover:bg-blue-600/80 transition-colors"
                onClick={() => {
        ...
        {isUpdateClickable ? (
          <button
            ...
            className={`transition-colors disabled:opacity-50 tabular-nums ${
              updateAvailable
                ? "text-blue-500 hover:text-blue-500/80 font-medium"
