...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState, warning: bool) -> &'static str {
    if warning && state == TrayIconState::Idle {
        return match theme {
            ...
            AppTheme::Colored => "resources/superflow.png",
        };
    }
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/superflow.png",
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("SuperFlow v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("SuperFlow v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
mod tests {
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "superflow-1.wav".to_string(),
            timestamp: 0,
            saved: false,
