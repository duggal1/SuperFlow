        ...
        Ok(output) => {
            ...
            if let Some(syntax) = classify_ydotool_key_syntax(&help) {
                *YDOTOOL_KEY_SYNTAX.get_or_init(|| {
                ...
                // Preserve SuperFlow's existing behavior and compatibility with current ydotool.
                log::warn!(
...
fn type_text_via_xdotool(text: &str) -> Result<(), String> {
    ...
    // Release both sides of SuperFlow's supported push-style modifiers to clear any
