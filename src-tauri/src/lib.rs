...
fn initialize_core_logic(app_handle: &AppHandle) {
    ...
    // Consistent everywhere: left click opens the app window, right click
    // shows the menu.
...
pub fn run(cli_args: CliArgs) {
    ...
    // upstream residency behavior with SUPERFLOW_METAL_RESIDENCY=1.
    #[cfg(target_os = "macos")]
    if std::env::var("SUPERFLOW_METAL_RESIDENCY").as_deref() == Ok("1") {
    ...
    // Single-instance forwards CLI args to an already-running SuperFlow and exits.
