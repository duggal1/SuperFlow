...
fn init_gtk_layer_shell(overlay_window: &tauri::webview::WebviewWindow) -> bool {
    if env_flag_enabled("SUPERFLOW_NO_GTK_LAYER_SHELL") {
        debug!("Skipping GTK layer shell init (SUPERFLOW_NO_GTK_LAYER_SHELL is enabled)");
        return false;
    }
...
/// with a copy affordance, so a dictation that landed nowhere (no focused input)
/// is still one click from the clipboard. Resizes/repositions via the shared
/// geometry path ("result" state), then hands the text to the overlay webview.
pub fn show_result_overlay(app_handle: &AppHandle, text: String) {
    show_overlay_state(app_handle, "result");
    // Queued after show_overlay_state's own main-thread hop, so the resize lands
    // before the payload does and the card renders at its final size.
    let handle = app_handle.clone();
    let _ = app_handle.run_on_main_thread(move || {
