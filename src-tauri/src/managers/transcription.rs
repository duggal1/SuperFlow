use crate::audio_toolkit::{
    apply_custom_words, detect_output_language, join_path_tokens, normalize_transcription_output,
...
impl TranscriptionManager {
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        if std::env::var("SUPERFLOW_FORCE_TRANSCRIPTION_FAILURE").is_ok() {
            return Err(anyhow::anyhow!(
                "Simulated transcription failure (SUPERFLOW_FORCE_TRANSCRIPTION_FAILURE)"
            ));
        }
...
/// Resolve how confidently SuperFlow knows the language of the text produced by a
...
fn resolve_output_language_evidence(
    ...
    // V3) always auto-detect and ignore SuperFlow's selection; transcribe-cpp also
...
/// Map SuperFlow's whisper accelerator setting to a transcribe-cpp [`Backend`].
///
