...
fn build_apple_intelligence_bridge() {
    ...
    // SUPERFLOW_FORCE_AI_STUB=1 is an explicit escape hatch: force the stub even when
    ...
    println!("cargo:rerun-if-env-changed=SUPERFLOW_FORCE_AI_STUB");
    let force_stub = env::var("SUPERFLOW_FORCE_AI_STUB").as_deref() == Ok("1");
    ...
    if command_line_tools_only && !force_stub {
        println!(
            "cargo:warning=Command Line Tools-only toolchain detected; Apple Intelligence \
             ...
             `sudo xcode-select -s /Applications/Xcode.app`, or set SUPERFLOW_FORCE_AI_STUB=1 to \
    ...
    let source_file = if has_foundation_models {
        ...
        // The SDK contains FoundationModels but we're still building stubs —
        // CLT-only detection or HANDY_FORCE_AI_STUB caused it, and either one
        // already announces itself above, so stay quiet here. Only claim the
        // SDK is "not found" when that's actually true.
        STUB_SWIFT_FILE
    } else {
